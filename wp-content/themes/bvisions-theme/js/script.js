document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Portfolio Filtering (Only run if filters exist) ---
    const portfolioFiltersContainer = document.querySelector('.portfolio-filters');
    if (portfolioFiltersContainer) {
        const filterButtons = portfolioFiltersContainer.querySelectorAll('.filter-button');
        ...
        filterButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                portfolioItems.forEach(item => {
                    ...
                    // Check if the item's categories contain the filter value, or if the filter is 'highlights' (which shows all items tagged 'highlights')
                    if (filterValue === 'highlights' && itemCategories && itemCategories.includes('highlights')) {
                         item.classList.remove('hide'); // Show item if filter is 'highlights' and item has 'highlights' category
                    } else if (filterValue !== 'highlights' && itemCategories && itemCategories.includes(filterValue)) {
